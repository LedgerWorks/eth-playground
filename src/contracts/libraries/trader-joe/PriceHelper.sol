...
import {TraderJoeConstants} from "./TraderJoeConstants.sol";
...
library PriceHelper {
  ...
  function getBase(uint16 binStep) internal pure returns (uint256) {
    unchecked {
      return
        TraderJoeConstants.SCALE +
        (uint256(binStep) << TraderJoeConstants.SCALE_OFFSET) /
        TraderJoeConstants.BASIS_POINT_MAX;
    }
  }
  ...
  function convertDecimalPriceTo128x128(uint256 price) internal pure returns (uint256) {
    return price.shiftDivRoundDown(TraderJoeConstants.SCALE_OFFSET, TraderJoeConstants.PRECISION);
  }
  ...
  function convert128x128PriceToDecimal(uint256 price128x128) internal pure returns (uint256) {
    return
      price128x128.mulShiftRoundDown(TraderJoeConstants.PRECISION, TraderJoeConstants.SCALE_OFFSET);
  }
}
