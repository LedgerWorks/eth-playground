...
export const coinFlipBan = async (options: TransactionOptions) => {
  ...
  const tx = await contract.populateTransaction.banPlayer(playerAddress);
  tx.type = 2;
...
export const register = (): void => {
  program
    ...
    .action((options: Partial<TransactionOptions>) => {
      return wrapAction(coinFlipBan, options);
    });
};
